...
DETECTOR_WEIGHTS = "fasterrcnn_braille_detection_best.pth"
CLASSIFIER_WEIGHTS = "vit_braille_classification_best.pth"
NUM_DET_CLASSES = 27  # 26 letters + background, matches object_detection.py training
...
def build_detector():
    ...
    print(f"  replaced box predictor head for {NUM_DET_CLASSES} classes")
    print(f"loading detector weights from {DETECTOR_WEIGHTS}")
    model.load_state_dict(torch.load(DETECTOR_WEIGHTS, map_location=DEVICE))
...
def detect_boxes(detector, image):
    ...
    keep_score = output["scores"] >= DETECTION_SCORE_THRESHOLD
    boxes = output["boxes"][keep_score]
    scores = output["scores"][keep_score]
    keep_nms = nms(boxes, scores, DETECTION_NMS_IOU_THRESHOLD)
    return boxes[keep_nms].cpu()
...
def process_image(detector, classifier, image_path: Path, output_path: Path) -> str:
    ...
    text = "".join(letters)
    print(f"decoded text (raw letters): {text}")

    # No word-boundary detection yet (detector only finds letter cells, not blank
    # gaps), so the whole run goes through liblouis as one word. Grade-2 contractions
    # can misparse across what should have been a word gap until that's added.
    braille_cells = liblouis.letters_to_braille_cells(text)
    translated_text = liblouis.braille_to_text(braille_cells)
    print(f"decoded text (liblouis grade-2 back-translation): {translated_text!r}")
